...
def loss(z_mean,  z_logvar  , recon_mean, recon_logvarinv, input):
    ...
    with tf.name_scope('loss'): 
        ...
        tf.summary.scalar('ce_loss',mean_l2_loss)
        	
        t_loss = tf.reduce_mean(z_loss + l2_loss)
        tf.summary.scalar('t_loss',t_loss)
    return t_loss, meadim_z_loss, mean_l2_loss

def loss_new(z_mean,  z_logvar  , recon_mean, recon_logvarinv, input):
    
    with tf.name_scope('loss'): 
        eps = tf.random_normal(shape=tf.shape(z_mean))
        z= eps * tf.exp(z_logvar * .5) + z_mean
        logpz = log_normal_pdf(z, 0., 0.)

        cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(logits=recon_mean, labels=input)
        logpx_z = -tf.reduce_sum(cross_ent, axis=[1, 2, 3])

        logqz_x = log_normal_pdf(z, z_mean, z_logvar)

    return -tf.reduce_mean(logpx_z + logpz - logqz_x), -tf.reduce_mean(logpz), -tf.reduce_mean(logpx_z-logqz_x)
